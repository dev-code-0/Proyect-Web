...
const DeepSpaceBackground = () => {
    // Memoizamos para evitar que Math.random() re-ejecute en cada render
    ...
    return (
        <div className="ar-ethereal-bg">
            ...
            {orbs.map(orb => (
                <div key={`orb-${orb.id}`} className="ar-orb"
                     style={{
                         width: `${orb.size}px`, height: `${orb.size}px`,
                         left: `${orb.left}%`, top: `${orb.top}%`,
                         animationDuration: `${orb.duration}s`, animationDelay: `${orb.delay}s`
                     }}
                />
            ))}
            {stars.map(star => (
                <div key={`star-${star.id}`} className="ar-star"
                     style={{
...
// ==========================================
// COMPONENTE: CARRUSEL 3D CILÍNDRICO
// ==========================================
            ...
            <div className="ar-cylinder-spinner" ref={spinnerRef}>
                {displayPhotos.map((src, i) => (
                    <div key={i} className="ar-cylinder-item"
                        ...
                        <img src={src} alt={`Memoria ${i + 1}`} draggable="false" />
                    </div>
                ))}
            </div>
        </div>
...
// ==========================================
export default function AppRecuerdos({ data }) {
    ...
    const logoTime = photos[0];
    const mainPhoto = LogoDisk; 
    ...
    const fallbackLockCode = targetDateMatch
        ? { day: Number(targetDateMatch[3]), month: Number(targetDateMatch[2]), year: Number(targetDateMatch[1]) % 100 }
        : { day: 26, month: 5, year: 23 };

    // Razones: admite string multilínea (una por línea) o array
    ...
    const reasons = Array.isArray(rawReasons) && rawReasons.length > 0
        ...
        : typeof rawReasons === 'string' && rawReasons.trim()
            ...
            : ['Tu sonrisa hermosa', 'Tus abrazos cálidos', 'Cómo me haces reír', 'Tu forma de ver el mundo', 'Lo seguro que me siento', 'Tu voz suave'];
    ...
    // Refs para stagger reveal al desbloquear
    const revealGallery = useRef(null);
    const revealMusic = useRef(null);
    const revealTime = useRef(null);
    const revealReasons = useRef(null);
    const revealAction = useRef(null);

    // Lógica del Candado
    useEffect(() => {
        if (
            code.day === config.lockCode.day &&
    ...
    // Stagger reveal al desbloquear — dispara la animación por secciones
    useEffect(() => {
        if (!isUnlocked) return;
        const sections = [revealGallery, revealMusic, revealTime, revealReasons, revealAction];
        const timers = sections.map((ref, i) =>
            setTimeout(() => ref.current?.classList.add('ar-visible'), i * 180)
        );
        return () => timers.forEach(clearTimeout);
    }, [isUnlocked]);

    const changeValue = useCallback((type, amount, max) => {
        setCode(prev => {
    ...
    const updateSongProgress = () => {
        ...
        setSongProgress(Number.isFinite(audio.currentTime) ? audio.currentTime : 0);
    };
    ...
    const seekBySeconds = (seconds, e) => {
        ...
        requestSeek(Math.min(Math.max(current + seconds, 0), maxTime));
    };

    const restartSong = (e) => { e.preventDefault(); e.stopPropagation(); requestSeek(0); };
    ...
    const handleScreenClick = (e) => {
        ...
        setClickHearts(prev => [...prev, { id, x: e.clientX - rect.left, y: e.clientY - rect.top }]);
        setTimeout(() => setClickHearts(prev => prev.filter(h => h.id !== id)), 1200);
    };
        ...
        <div className="ar-app-container" onClick={handleScreenClick}>
            ...
            {!isUnlocked ? (
                ...
                <div className="ar-screen-main ar-hide-scrollbar">
                    ...
                    {/* GALERÍA 3D CILÍNDRICA */}
                    <div ref={revealGallery} className="ar-gallery-container ar-reveal" onClick={(e) => e.stopPropagation()}>
                        <h2 className="ar-section-header">Recuerdos Inolvidables</h2>
                        <CylinderCarousel photos={config.photos} />
                    </div>

                    {/* TARJETA: MÚSICA */}
                    <div ref={revealMusic} className="ar-glass-card ar-reveal" onClick={(e) => e.stopPropagation()}>
                    ...
                    {/* TARJETA: TIEMPO */}
                    <div ref={revealTime} className="ar-glass-card ar-reveal" onClick={(e) => e.stopPropagation()}>
                    ...
                    {/* TARJETA: RAZONES */}
                    <div ref={revealReasons} className="ar-glass-card ar-reveal" style={{marginBottom: '1rem'}}>
                    ...
                    {/* BOTÓN CARTA */}
                    <div ref={revealAction} className="ar-action-section ar-reveal" onClick={(e) => e.stopPropagation()}>
            ...
            {isLetterOpen && (
                <div className="ar-modal-overlay" onClick={() => setIsLetterOpen(false)}>
                    <div className="ar-modal-card" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="ar-modal-signature-title">~ Con amor, {config.nombre} ~</div>
                    </div>
                </div>
            )}
        </div>
