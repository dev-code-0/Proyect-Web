...
import bouquetImage from "./images/ramo.svg";
...
const FLOATING_HEARTS_POSITIONS = [10, 22, 35, 48, 60, 72, 85];
const TOTAL_SECTIONS = 3;
const POLAROID_ROTATIONS = ["-6deg", "2deg", "7deg", "-4deg", "5deg", "-8deg", "3deg"];
...
// ─────────────────────────────────────────
// HeartSVG — reemplaza el SVG duplicado x2
// ─────────────────────────────────────────
function HeartSVG({ color, letter, rotate = 0 }) {
  return (
    <svg
      ...
      width="68"
      height="68"
      viewBox="0 0 728.662698 734.909493"
      preserveAspectRatio="xMidYMid meet"
      style={{ transform: `rotate(${rotate}deg)`, margin: "0 -5px" }}
      aria-hidden="true"
    >
      <g
        transform="translate(-143.017164,851.387642) scale(0.100000,-0.100000)"
        fill={color}
        stroke="none"
      >
...
// ─────────────────────────────────────────
// PolaroidStack — stack visual con CSS, sin duplicar el DOM
// ─────────────────────────────────────────
...
function ArrowButton({ onClick, visible, direction = "down", ariaLabel }) {
  ...
  return (
    ...
    >
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"
        ...
        <path d={isUp ? "M12 19V5M5 12l7-7 7 7" : "M12 5v14M5 12l7 7 7-7"} />
      </svg>
    </button>
  );
}
...
function HeroPanel({ recipientName, recipientInitial, senderInitial, giftImage, onNext, active }) {
  ...
  return (
    <section ref={scrollRef} className={`panel-fl panel-hero-fl section-slide-fl${active ? " section-slide--in-fl" : " section-slide--out-fl"}`} aria-label="Portada">
      ...
      <ArrowButton onClick={onNext} visible={active} />
    </section>
  );
}
...
// DistancePanel
// ─────────────────────────────────────────
function DistancePanel({ message, onNext, onPrev, active }) {
  const scrollRef = useRef(null);
...
// LetterPanel
// ─────────────────────────────────────────
function LetterPanel({ senderName, recipientName, loveLetter, photoUrls, photoCaption, onPrev, active }) {
  const scrollRef = useRef(null);
...
// ─────────────────────────────────────────
// ROOT — orquestador principal
// ─────────────────────────────────────────
export default function FloresAmarillasTemplate({ data = {} }) {
  const recipientName   = data.recipientName     ?? data.nombre    ?? "Ma...";
  const senderName      = data.senderName        ?? data.de        ?? "Lu...";
  const giftImage       = data.giftImageUrl      ?? data.giftImage ?? Ilustracion;
  ...
  const photosToShow    = photoUrls.length > 0 ? photoUrls : [Fotos];
  const photoCaption    = data.photoCaption      ?? data.caption   ?? "Nuestro momento especial";
  ...
  const inicial_1 = String(recipientName || "").trim().charAt(0).toUpperCase() || "M";
  const inicial_2 = String(senderName || "").trim().charAt(0).toUpperCase() || "L";

  const [current, setCurrent] = useState(0);
  const [transitioning, setTransitioning] = useState(false);

  function goNext() {
    if (transitioning || current >= TOTAL_SECTIONS - 1) return;
    setTransitioning(true);
    // Espera que termine el fade-out (600ms) antes de cambiar la sección
    setTimeout(() => {
      setCurrent(c => c + 1);
      ...
      <div className={`section-wrapper-fl${transitioning ? " section-wrapper--fading-fl" : ""}`}>
        {current === 0 && (
          <HeroPanel
            ...
            recipientInitial={inicial_1}
            senderInitial={inicial_2}
            giftImage={giftImage}
            onNext={goNext}
